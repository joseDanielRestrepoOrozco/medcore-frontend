...
type Template = { id?: string; title?: string; description?: string; fields: Field[] };
...
const MedicalHistoryNew = () => {
  ...
  // currently no UI to edit symptoms; keep empty string placeholder
  const symptoms = '';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setSelectedTemplate(res.data?.template || res.data || null);
      } catch (err: unknown) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fd.append('description', selectedTemplate?.description || description || summary);

      // Campos que podría contener el template
      const known = ['title', 'description', 'symptoms', 'diagnosis', 'treatment', 'observations', 'nextAppointment'];
      ...
      if (selectedTemplate) {
        ...
        for (const k of Object.keys(templateValues)) {
          ...
          if (known.includes(k)) {
            ...
            if (k === 'nextAppointment' && v) fd.append('nextAppointment', String(v));
      ...
      const obsFromTemplate = (templateValues as Record<string, unknown>)?.observations;
      if (obsFromTemplate) {
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4">
        <div>
          ...
          <div className="mt-2 flex items-center gap-2">
            <select title="Seleccionar template" value={selectedTemplateId ?? ''} onChange={(e) => setSelectedTemplateId(e.target.value || null)} className="border rounded px-3 py-2">
        ...
        {selectedTemplate ? (
          <div className="mt-4">
            <DynamicForm
              ...
              initial={{}}
              onSubmit={async (vals) => { setTemplateValues(vals); /* store until final submit */ }}
            />
            <div className="text-sm text-slate-500 mt-2">Completa el template y pulsa "Guardar" en el formulario del template para validar los campos; luego haz clic en el botón principal "Guardar" para subir archivos y completar la consulta.</div>
          </div>
        ) : null}
        <div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <input title="Título" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full border rounded px-3 py-2" />
          </div>
          <div>
            <label className="block text-sm font-medium">Fecha</label>
            <input title="Fecha" type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full border rounded px-3 py-2" />
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          >
            Arrastra y suelta los archivos aquí o usa el botón para seleccionarlos
          </div>
          <div className="mt-2">
            <button type="button" onClick={() => fileInputRef.current?.click()} className="px-3 py-2 bg-white border rounded">Seleccionar archivos</button>
            <input ref={fileInputRef} type="file" multiple accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => setFiles((prev) => [...prev, ...Array.from(e.target.files || [])])} className="sr-only" title="Seleccionar archivos" />
          </div>
          {files.length > 0 && (
